  ...
  class Day5
  {
    public static (int, int) Solve()
    {
      ...
      foreach (string printing in prints)
      {
        ...
        if (ApplyRules(printOrder, rules))
        {
          validMids += int.Parse(printOrder[printOrder.Count / 2]);
        }
        else
        {
          fixedMids += Fix(printOrder, rules);
        }
      }
    ...
    static int Fix(List<string> printOrder, Dictionary<string, List<string>> rules)
    {
      ...
      {
        var curNum = printOrder[i];
        var curRules = rules.GetValueOrDefault(key: curNum, []);
        bool noInsert = true;
        for (int y = 0; y < newOrder.Count; y++)
        {
          if (curRules.Contains(newOrder[y]))
          {
            newOrder.Insert(y, curNum);
            noInsert = false;
            break;
          }
        }
    ...
    static (Dictionary<string, List<string>>, string[]) CreateRules(string[] rules)
    {
      ...
      for (i = 0; i < rules.Length; i++)
      {
        ...
        if (ruleMap.TryGetValue(parts[0], out List<string>? value))
        {
          value.Add(parts[1]);
        }
        else
        {
          ruleMap.Add(parts[0], [parts[1]]);
        }
      }
